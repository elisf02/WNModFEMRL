...
WN_MCMC_and_Simu <- function(seed=0,
                             ...
                             file_temperatura = "per_mcmc/MeanTemp_",
                             file_numero_pool = "per_mcmc/TotPool_",
                             file_numero_pool_positivi = "per_mcmc/TotPoolPos_",
                             file_pool_size = "per_mcmc/MeanCulex_",
  ...
  for(quale_cluster in 1:numero_cluster){
    for(anno in anno_inizio:anno_fine){
      ...
      to_terminal=paste("time",
                        paste0(system.file(package = "WNModFEMRL"),'/templates/Uccelli_WNV_mcmc'),
                        ...
                        beta,quale_cluster,prior_pR,prior_B0,
                        min_niB,max_niB,min_recB,max_recB,min_b0,max_b0,min_s,max_s,
                        min_phi,max_phi,numero_cluster)
      system(to_terminal)
      ...
      to_terminal = paste("time",
                          paste0(system.file(package = "WNModFEMRL"),'/templates/Uccelli_WNV_simulation'),
                          seed,file_temperatura,file_numero_pool,file_numero_pool_positivi,
...
WN_PlotModelFit <- function(anno_inizio,
                            anno_fine,
                            numero_cluster,
                            ...
                            file_numero_pool="totale_pool_cluster_WNV5reg_V2_70trappole_",
                            file_numero_pool_positivi = "totale_pool_WNVpos_cluster_WNV5reg_V2_70trappole_",
                            file_pool_size="mean_pool_size_cluster_WNV5reg_V2_70trappole_",
  ...
  for(anno in anno_inizio:anno_fine){
    ...
    jpeg(nome_file_plot,height=plot_on_row*1000,width=plot_on_col*1400,res=200)
    par(mfrow=c(plot_on_row,plot_on_col),mar=margins_plot,cex.main=cex_main,cex.axis=cex_axes)
    ...
    for(quale_cluster in 1:numero_cluster){
      ...
      points(1:xmax,numero_pool_positivi[quale_cluster, quando_ho_pool][1:xmax],pch=19,col="darkorange",cex=2)
...
WN_PlotDynamics <- function(anno_inizio,
  ...
  for(anno in anno_inizio:anno_fine){
    ...
    nome_file_plot=paste0(FolderPlotOut, PlotName ,anno,".jpg")
    jpeg(nome_file_plot,height=1*1000,width=2*1400,res=200)

    par(mfrow=c(1,2),mar=margins_plot,cex.main=cex_main,cex.axis=cex_axes)
    ...
    for(quale_cluster in 1:numero_cluster){
      ...
      axis(1,at=c(1,1+31,1+31+30,1+31+30+31,1+31+30+31+31,1+31+30+31+31+30),
           labels=mesi,cex.axis=1.5,las=2)
      ...
      if(what_plot == 'bird_population')
        text = 'Bird pop'
      mtext(text,side=2,line=6.5,adj=0.5)
    }
    dev.off()
  }
}
